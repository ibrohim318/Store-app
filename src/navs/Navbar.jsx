...
function Navbar() {
    ...
    return (
        <div className={`relative ${menu ? "w-55 transition-all duration-200" : "w-17 transition-all duration-200"}  h-screen border-r border-gray-200 dark:border-gray-500 bg-white dark:bg-[#1e293b] flex flex-col`}>
            {/* Logo */}
            <div className="flex items-center gap-2 p-3 border-b border-gray-200 dark:border-gray-500">
            ...
            <button onClick={toggleMenu} className={`absolute top-15 cursor-pointer -right-3 w-6 h-6 rounded-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-500 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 shadow-sm transition-colors ${menu ? "transition-all duration-200" : "rotate-180 transition-all duration-200"}`} title="Yig'ish"> <FaChevronLeft size={10} /></button>
            {/* menus */}
            <div className="px-2">
                ...
                <div>
                    {menus?.map(elem => {
                        ...
                        return (
                            <NavLink key={elem.name} to={elem.path} className={({ isActive }) => `w-full h-[38px] flex items-center gap-3 px-2 rounded-lg transition-colors text-md font-mono mt-1 ${isActive ? "bg-[#eef2ff] dark:bg-[#1e2a5e] text-[#433dcb] dark:text-indigo-300" : "text-gray-400 dark:text-gray-500 hover:text-black dark:hover:text-white"}`}> <Icon className={`${menu ? " text-lg" : "ml-2 text-lg"} `} /> <h3>{menu ? elem.name : ""}</h3></NavLink>
                        );
                    })}
                </div>
            </div>
            ...
            {/* settings and user */}
            <div className="mt-auto border-t border-gray-200 dark:border-gray-500">
